...
from karaoke.infra.whisperx_client import call_whisperx_endpoint_manual
...
def create(
    ...
    try:
        ...
        logger.warning(
            f"Error na normalización: {erro_normalizacion}, continuando co video original"
        )
    ...
    whisper_response = call_whisperx_endpoint_manual(
        ruta_voz, letras_normalizadas, None, enable_diarization, hf_token, whisper_model
    )
    ...
    grupos_texto = group_word_segments_automatic(
        segmento_palabras, max_words_per_phrase=6, max_duration=3.5
    )
    if not grupos_texto:
        return ""
    ...
    for indice, grupo in enumerate(grupos_texto):
        ...
        clip_texto = create_karaoke_text_clip(
            grupo, next_line_info=linha_seguinte, advance=0.5, duration_padding=0.5
        )
    ...
    try:
        ...
        ruta_vocal_output = os.path.join(
            "./output", f"vocal_{whisper_model}_{nome_sin_extension}.wav"
        )
        ruta_instrumental_output = os.path.join(
            "./output", f"instrumental_{whisper_model}_{nome_sin_extension}.wav"
        )
...
def create_with_manual_lyrics(
    ...
    whisper_response = call_whisperx_endpoint_manual(
        ruta_voz, letras_normalizadas, language, enable_diarization, hf_token, whisper_model
    )
    archivoSRT = ruta_voz.replace(".wav", f"_whisperx_{whisper_model}.srt")
    ...
    try:
        ...
        if video_path != ruta_video_orixinal:
            try:
                video_fondo = (
                    VideoFileClip(ruta_video_orixinal)
                    .set_duration(audio_mesturado.duration)
                    .set_fps(30)
                )
...
def generate_instrumental(
    ...
    if not video_path.lower().endswith(".mp3"):
        try:
            ...
            logger.warning(
                f"Error na normalización: {erro_normalizacion}, continuando co video orixinal"
            )
