...
from karaoke.domain.render_config import (
    ...
    COR_TEXTO,
    ESPACIADO_LIÑAS,
    FACTOR_ACELERACION_MAX,
    FACTOR_FONTE_LIÑA_SEGUINTE,
    FONTE,
    GROSOR_CONTORNO_TEXTO,
    MODO_SILABICO,
    MOSTRAR_LIÑA_SEGUINTE,
    PADDING_FONDO_TEXTO,
    PESO_PROGRESO_SEGMENTO,
    PESO_PROGRESO_TEMPORAL,
    TAMAÑO_FONTE,
    TAMAÑO_FONTE_MIN,
    TEXT_CLIP_WIDTH,
    UMBRAL_ACELERACION,
...
def render_line_image(
    line_info: dict,
    t_offset: float,
    clip_width: int = TEXT_CLIP_WIDTH,
    font_path: str = FONTE,
    ...
    for palabra in palabras:
        ...
        ancho_texto = (
            fonte.getlength(liñaPrueba)
            if hasattr(fonte, "getlength")
            else fonte.getsize(liñaPrueba)[0]
        )
    ...
    altura_liña = int(
        tamaño_fonte_dinamico * 1.4
    )  # puxen un espaciado de linea un 40% mais alto ca o tamaño da fonte
    ...
    capa_fondo = Image.new(
        "RGBA", (clip_width, altura_total), (0, 0, 0, 0)
    )  # fondo +- transparente
    ...
    for liña in lineasAjustadas:
        ancho_liña = (
            fonte.getlength(liña) if hasattr(fonte, "getlength") else fonte.getsize(liña)[0]
        )  # serviume esto para o problema do ancho. REVISAR A REFERENCIA EN INTERNET PARA AÑADILA NA MEMORIA!!!
    ...
    if hasattr(debuxar_fondo, "rounded_rectangle"):
        ...
        debuxar_fondo.rectangle(
            [(x_fondo, 0), (x_fondo + ancho_fondo, altura_total)], fill=COR_FONDO_TEXTO
        )
    ...
    if MODO_SILABICO and line_info.get("words"):
        ...
        if duracion_total_liña > 0:
            ...
            progreso_segmento = (
                (segmentos_pasados / len(line_info["words"])) if line_info["words"] else 0.0
            )
            ...
            progreso_combinado = (progreso_temporal * PESO_PROGRESO_TEMPORAL) + (
                progreso_final * PESO_PROGRESO_SEGMENTO
            )
        else:
        ...
        if progreso_combinado > UMBRAL_ACELERACION:
            ...
            silabas_para_resaltar = min(
                total_silabas, int(progreso_combinado * total_silabas * factor_aceleracion)
            )
        ...
        for i, liña in enumerate(lineasAjustadas):
            ...
            ancho_liña = (
                fonte.getlength(liña) if hasattr(fonte, "getlength") else fonte.getsize(liña)[0]
            )  # fajo un cálculo do ancho total da linea para o tema de que este centrada
            ...
            for idx_palabra, palabra in enumerate(palabras_liña):
                ...
                if len(silabas_palabra) <= 1:
                    ...
                    debuxar.text(
                        ...
                        font=fonte,
                        fill=cor,
                        stroke_width=GROSOR_CONTORNO_TEXTO,
                        stroke_fill=COR_CONTORNO_TEXTO,
                    )
                    ancho_palabra = (
                        fonte.getlength(palabra)
                        if hasattr(fonte, "getlength")
                        else fonte.getsize(palabra)[0]
                    )
                    x += ancho_palabra
                    indice_silaba += 1
                else:
        ...
        for i, liña in enumerate(lineasAjustadas):
            ...
            for idx_palabra, palabra in enumerate(palabras_liña):
                ...
                x += ancho_palabra + (
                    fonte.getlength(" ") if hasattr(fonte, "getlength") else fonte.getsize(" ")[0]
                )
...
def render_next_line_image(
    ...
    if len(texto_completo) > 100:
        factor_reduccion = min(1.0, 100 / len(texto_completo))
        tamaño_fonte_dinamico = max(
            TAMAÑO_FONTE_MIN * FACTOR_FONTE_LIÑA_SEGUINTE, int(font_size * factor_reduccion)
        )
    ...
    x_fondo = (clip_width - ancho_fondo) // 2
    debuxar_fondo.rectangle(
        [(x_fondo, 0), (x_fondo + ancho_fondo, altura_total)], fill=colorDeFondoDaSiguienteLinea
    )
    ...
    for i, liña in enumerate(lineasAjustadas):
        ...
        debuxar.text(
            ...
            liña,
            font=fonte,
            fill=COR_LIÑA_SEGUINTE,
            stroke_width=int(GROSOR_CONTORNO_TEXTO * 0.75),
            stroke_fill=COR_CONTORNO_TEXTO,
        )
...
def create_karaoke_text_clip(
    ...
    def facer_frame(t):
        ...
        if MOSTRAR_LIÑA_SEGUINTE and next_line_info:
            ...
            frame_combinado[
                y_seguinte_liña : y_seguinte_liña + altura2,
                offset_x2 : offset_x2 + frame_seguinte.shape[1],
            ] = frame_seguinte
