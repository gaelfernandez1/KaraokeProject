...
def get_optimal_demucs_args(gpu_info):
    ...
    if gpu_info["recommended_device"] == "cuda":
        base_args.extend(["--device", "cuda", "--shifts", "1", "--overlap", "0.25"])
        ...
        base_args.extend(["--device", "cpu", "--shifts", "1", "--jobs", "2", "--segment", "10"])
...
def print_system_summary():
    ...
    if gpu["cuda_available"]:
        ...
        logger.info(f"Memoria GPU: {gpu['gpu_memory']:.1f}GB")
